...
class TwitterGraph {
  ...
  void draw() {
    ...
    for (int i = 0; i < springs.size(); i++) {
      ...
      if (dist(dataX, dataY, s.getFromNode().x, s.getFromNode().y) < from.savedDiameter) {
        from.diameter = from.savedDiameter*10;
        println("X: " + from.x + " Y:  " + from.y);
      } else {
    ...
    stroke(255, 40);
    fill(averageSentiment);
    //noFill(); // REMOVE THIS WHEN SENTIMENT IS WORKING 
